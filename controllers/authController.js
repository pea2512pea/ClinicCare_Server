        ...
        jwt.verify(token, process.env.JWT_SECRET, (err, user) => {
            ...
            if (user.role === 'patient') {
                PatientModel.findByPk(user.id, {
                    ...
                    .catch(() => res.status(500).json({ error: 'Failed to retrieve patient' }));
            } else if (user.role === 'doctor') {
                DoctorModel.findByPk(user.id, {
                    attributes: ['first_name', 'last_name', 'email', 'phone', 'specialty']
                })
                    .then(doctor => {
                        if (doctor) {
                    ...
                    .catch(() => res.status(500).json({ error: 'Failed to retrieve doctor' }));
            } else {
...
const registerPatient = async (req, res) => {
    try {
        const { citizen_id, first_name, last_name, phone, email, password } = req.body;
